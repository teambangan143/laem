...
$topstatus="Thank You For registering your product code. It will now accomulate in your Uni-Level Point Value ;)";
...

{
 ...
 $ear="$row[tamount]";
  ...
  //mysqli_query($con,"INSERT into `affiliateuserpackage` (memberID,doj,pcktakenID) VALUES ('$aid',NOW(),'$renewpckid')");
  
  
  mysqli_query($con,"UPDATE prodcodereg SET status=1, regdate=NOW(), regby='$userid' WHERE prodcode='$prodcode'");
...
<?php /*?><?php $query="SELECT id,fname,email,doj,active,username,address,pcktaken,tamount FROM  affiliateuser where username = '$userid'"; 
...
<form action="renewgatewayck.php" method="post">
    ...
    <input type="hidden" name="username" value="<?php print $userid ?>">
